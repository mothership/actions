...
export default class Server implements Hub.RouteBuilder {
  static run() {
    ...
    blocked(
      (time: number, stack: string[]) => {
        winston.warn(
          `Event loop blocked for ${time}ms, operation started here:\n${stack.join(
            "\n",
          )}`,
        )
      },
    ...
    const timeout = process.env.ACTION_HUB_SOCKET_TIMEOUT
      ? parseInt(process.env.ACTION_HUB_SOCKET_TIMEOUT, 10)
      : 0
  ...
  constructor() {
    ...
    this.route("/", async (req, res) => {
      ...
      winston.debug(
        `version: ${version}, response: ${this.actionList[version]}`,
      )
    })
    ...
    this.route(
      ...
      this.jsonKeepAlive(async (req, complete) => {
        const request = Hub.ActionRequest.fromRequest(req)
        const action = await Hub.findAction(req.params.actionId, {
          lookerVersion: request.lookerVersion,
        })
        const queue = action.extendedAction
          ? extendedJobQueue
          : expensiveJobQueue
    ...
    this.app.get("/actions/:actionId/oauth", async (req, res) => {
      ...
      if (isOauthAction(action)) {
        ...
        const url = await (action as OAuthAction).oauthUrl(
    ...
    this.app.get("/actions/:actionId/oauth_redirect", async (req, res) => {
      ...
      if (isOauthAction(action)) {
        try {
          await (action as OAuthAction).oauthFetchInfo(
            req.query,
            this.oauthRedirectUrl(action),
          )
          res.statusCode = 200
          res.send(
            `<html><script>window.close()</script>><body>You may now close this tab.</body></html>`,
          )
        } catch (e) {
  ...
  private oauthRedirectUrl(action: Hub.Action) {
    return this.absUrl(
      `/actions/${encodeURIComponent(action.name)}/oauth_redirect`,
    )
  }
  /**
  ...
  private jsonKeepAlive(
    fn: (req: express.Request, complete: (data: any) => void) => Promise<void>,
  ...
  private route(
    ...
    this.app.post(urlPath, async (req, res) => {
      ...
      const tokenMatch = headerValue
        ...
        : undefined
      if (1 < 0) {
        console.log(tokenMatch, apiKey)
        //!tokenMatch || !apiKey.validate(tokenMatch[1])
        res.status(403)
